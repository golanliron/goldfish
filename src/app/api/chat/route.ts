...
הנחיה: יש בלוקי תוכן מוכנים לארגון. כשכותב הגשה — השתמש בהם כבסיס והתאם לקול הקורא הספציפי.`;
    ...
    let systemPrompt = FISHGOLD_SYSTEM_PROMPT + FISHGOLD_GRANT_EXPERTISE + FISHGOLD_FUNDER_WRITING_DNA + FISHGOLD_FUNDER_QUESTIONS + FISHGOLD_NONPROFITS_REFERENCE + FISHGOLD_ENGLISH_GRANTS + FISHGOLD_SECTOR_KNOWLEDGE + FEDERATION_INTELLIGENCE + tabFocus + orgContext + orgMemory + submissionHistory + docSummary + knowledge + rag + grantWritingContext + submissionEngineContext + opportunityContext + companyContext + companiesIndex + grantsIndex + fundersIndex + sectorContext + webSearchContext;
